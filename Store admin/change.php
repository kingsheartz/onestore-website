...
</div><div class="imscr" id="imsrc<?=$row['item_description_id']?>" onscroll="movefr('imsrc<?=$row['item_description_id']?>')">
...
<td><?=$row1['internal_storage_name']?></td></tr>
<?php
...
$row1=$st1->fetch(PDO::FETCH_ASSOC);
?>
<tr><th>Brand</th><td><?=$row1['brand_name']?></td></tr>
<?php
...
<input type="number" name="price" onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57"value="<?=$row['price']?>" required="" class="form-control">
</div>
<div class="form-group">
<label class="floating-label">Quantity</label>
<input type="number" name="quantity" onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57" required="" value="<?=$row['quantity']?>" class="form-control" >
</div>
<div class="form-group">
<label class="floating-label">Order Preference</label>
<select name="pref" required="" class="form-control">

<option value="<?php if($row['order_preference']!=0){echo $row['order_preference'];}?>">
<?php
...
" onclick="showupda(<?=$row['item_description_id']?>,1)">
    <i class="fas fa-pencil-square" style="margin-right: 20px;float: left;font-size: 24px" ></i>Update</button>
<button  name="remove_data" style="float: right;
    background: #fd0018;
    margin-left: 12px;
    margin-right: 12px;" onclick="showupda(<?=$row['item_description_id']?>,0)"><i class="fas fa-trash" style="margin-right: 20px;float: left;font-size: 24px"></i>Remove</button>
</div>
