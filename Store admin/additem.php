...
.modal-header {
    ...
    background: -webkit-gradient(linear, left bottom, left top, color-stop(0, #000000), color-stop(1, #7221f3)) !important;    color: white;
...
.modal-footer {
 ...
 <i class="fa fa-plus-square" style="font-size: 24px;padding-right: 12px"aria-hidden="true"></i>Add Products</h4>
...
item_description.item_description_id NOT IN (SELECT item_description_id FROM product_details where store_id=$id ) GROUP BY item_description.item_id ORDER BY item_description.item_description_id DESC LIMIT 8";
$st=$pdo->query($query);
$ct='ab';
?>
<div class="difcat ">
...
<div class="middle">
   ...
   <form id="C<?=$row['item_description_id']?>" method="post" action="addsub.php" name="<?=$row['item_description_id']?>">
...
<button onclick="showupda(<?=$row['item_description_id']?>)"class="updation"><i class="fab fa-buffer" style="font-size: 24px;padding-right: 12px"aria-hidden="true"></i>Add</button>
...
item_description.item_description_id NOT IN (SELECT item_description_id FROM product_details where store_id=$id ) GROUP BY item_description.item_id";
...
while ($row=$st->fetch(PDO::FETCH_ASSOC)) {     
  ...
  justify-content: center;height: 200px;width:100%;background: white;text-align: center;"><img class="image" align="middle" src="../images/<?=$row['category_id']?>/<?=$row['sub_category_id']?>/<?=$row['item_description_id']?>.jpg"></div>
    ...
    <div class="middle">
       ...
       <form id="<?=$row['item_description_id']?>" method="post" action="addsub.php" name="<?=$row['item_description_id']?>">
