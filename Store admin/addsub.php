...
<div class="proupda ">
 ...
 <h4 style="text-overflow: ellipsis;
  ...
  <div class="row">
<?php
 $query="SELECT * FROM item JOIN item_description ON item.item_id=item_description.item_id where item_description.item_id=$it and  item_description.item_description_id not in(SELECT item_description_id FROM product_details where store_id=$id)";
$st=$pdo->query($query);
$tr=$st->rowCount();
if ($tr==0) {
  ?>
...
<input type="number" onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57" name="price" required="" class="form-control">
</div>
<div class="form-group">
<label class="floating-label">Quantity</label>
<input type="number" onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57" name="quantity" required="" class="form-control" >
</div>
<div class="form-group">
...
width: 200px;
    ...
    float: right;
    margin-right: 12px;
" onclick="newup(<?=$row['item_description_id']?>,1)"><i class="fas fa-save" style="margin-right: 20px;float: left;font-size: 24px" ></i>Add to Store</button>
