...
if (isset($_POST['uppr_id'])||isset($_POST['upim_url'])||isset($_POST['upname'])||
...
<div class="imgdis">
<form id="<?=$row['item_description_id']?>" target='ifr<?=$row['item_description_id']?>' method="post" name="<?=$row['item_description_id']?>" action="prosub.php" enctype="multipart/form-data" >

 <table><tr>
   <th class="ttd">
     <div class="thd ">Product</div>
   <div class="prim col-sm-12">
<img src="../images/<?=$row['category_id']?>/<?=$row['sub_category_id']?>/<?=$row['item_description_id']?>.jpg"  >
...
<?php
  ...
  ?>
<td rowspan=2 class="rwtd"> <div class="thd">Upload Images</div>
 <div class="col-sm-12">
...
<div class="image-upload">  
        <label for="<?=$row['item_description_id']?>file-input<?=$i?>" style="width: 100%; cursor: pointer;"><center>
        <img id="<?=$row['item_description_id']?>previewImg<?=$i?>" src="../images/<?=$row['category_id']?>/<?=$row['sub_category_id']?>/<?=$row['item_description_id']?>_<?=$i?>.jpg" 
        style="max-width: 150px;max-height: 150px;height: auto;width: auto;"> 
        
        </center>

              </label> 
     <input id="<?=$row['item_description_id']?>file-input<?=$i?>" type="file" name="my_file<?=$i?>" onchange="previewFile('#<?=$row['item_description_id']?>file-input<?=$i?>',<?=$i?>,<?=$row['item_description_id']?>);" style="display: none; cursor: pointer;">   
      </div>
<?php
}}
?>
...
<div class="imdeta col-sm-12" >
  <input type="number" maxlength="10" required="" class="form-control" name="nj"style="height: 30px;" onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= <?=$k?> && event.charCode <= 57" id="nj<?=$row['item_description_id']?>">
  ...
  background: -webkit-gradient(linear, left bottom, left top, color-stop(0, #151414), color-stop(1, #1d1d1d)) !important;
    ...
    font-size: 16px;
    padding: 2px;
    border-radius: 5px;" name="numi" onclick="numplusone(<?=$itcn?>,<?=$row['item_description_id']?>)"><i class="fas fa-plus"></i></button>
    ...
    font-size: 16px;
    padding: 2px;
    border-radius: 5px;" name="numk" onclick="numminone(<?=$itcn?>,<?=$row['item_description_id']?>)"><i class="fas fa-minus"></i></button>
 </div>
  </div>
</td>
</tr>
...
<iframe name='ifr<?=$row['item_description_id']?>' id="ifr<?=$row['item_description_id']?>" style='display: none;'></iframe>
<script>
