...
<body>
  <div class="wrapper">
    ...
    <body>
      ...
      $id = $_SESSION['id'];
      require "pdo.php";
      $query = "SELECT * FROM item JOIN item_description ON item.item_id=item_description.item_id where item_description.item_description_id NOT IN (SELECT item_description_id FROM product_details where store_id=$id ) GROUP BY item_description.item_id ORDER BY item_description.item_description_id DESC LIMIT 8";
      $st = $pdo->query($query);
      $ct = 'ab';
      ?>
      <div class="difcat ">
        ...
        <div class="difrow" id="difrow<?= $ct ?>" onscroll="movefr('difrow<?= $ct ?>')">
          ...
          ?>
            <?php
            $query = "SELECT * FROM item JOIN item_description ON item.item_id=item_description.item_id where item.category_id=$ct and item_description.item_description_id NOT IN (SELECT item_description_id FROM product_details where store_id=$id ) GROUP BY item_description.item_id";
            $st = $pdo->query($query);
            $product = $st->rowCount();
            if ($product == 0) {
              continue;
