$(function () {
    ...
    test("filtering=false prevents rendering filter template", function () {
        ...
        $.each(jsGrid.fields, function (name, FieldClass) {
            if (isFieldExcluded(FieldClass)) return;
            var field = new FieldClass({ filtering: false });
            equal(
                field.filterTemplate(),
                "",
                "empty filter template for field " + name
            );
        });
    });
    test("inserting=false prevents rendering insert template", function () {
        ...
        $.each(jsGrid.fields, function (name, FieldClass) {
            ...
            equal(
                ...
                "",
                "empty insert template for field " + name
            );
        });
    });
    test("editing=false renders itemTemplate", function () {
        ...
        $.each(jsGrid.fields, function (name, FieldClass) {
            ...
            var editTemplateContent =
                editTemplate instanceof jQuery
                    ? editTemplate[0].outerHTML
                    : editTemplate;
            var itemTemplateContent =
                itemTemplate instanceof jQuery
                    ? itemTemplate[0].outerHTML
                    : itemTemplate;
            equal(
                editTemplateContent,
                itemTemplateContent,
                "item template is rendered instead of edit template for " + name
            );
            equal(args.length, 2, "passed both arguments for " + name);
            equal(
                args[0],
                "test",
                "field value passed as a first argument for " + name
            );
            equal(
                args[1],
                item,
                "item passed as a second argument for " + name
            );
        });
    });
    module("jsGrid.field");
    ...
    test("basic", function () {
        ...
        equal(
            field.editTemplate("testEditValue")[0].tagName.toLowerCase(),
            "input"
        );
        strictEqual(field.filterValue(), "");
        strictEqual(field.insertValue(), "");
    ...
    test("set default field options with setDefaults", function () {
        ...
        equal(
            $element.jsGrid("option", "fields")[0].defaultOption,
            "test",
            "default field option set"
        );
    });
    ...
    test("basic", function () {
        var field, filterTemplate, insertTemplate, editTemplate;
    ...
    test("value type auto-defined", function () {
        ...
        strictEqual(
            field.sorter,
            "string",
            "sorter set according to value type"
        );
        field.filterTemplate();
        strictEqual(field.filterValue(), "2");
        field.editTemplate("2");
    ...
    test("value type defaulted to string", function () {
        var field = new jsGrid.SelectField({
            name: "testField",
            items: [{ text: "test1" }, { text: "test2", value: "2" }],
            textField: "text",
            valueField: "value",
        });
        strictEqual(
            field.sorter,
            "string",
            "sorter set to string if first item has no value field"
        );
    });
    ...
    test("basic", function () {
        ...
        equal(
            headerTemplate.filter("." + field.insertModeButtonClass).length,
            1
        );
        var $modeSwitchButton = headerTemplate.filter(
            "." + field.modeButtonClass
        );
        $modeSwitchButton.trigger("click");
        equal(
            headerTemplate.filter("." + field.searchModeButtonClass).length,
            1
        );
        filterTemplate = field.filterTemplate();
        equal(filterTemplate.filter("." + field.searchButtonClass).length, 1);
        equal(
            filterTemplate.filter("." + field.clearFilterButtonClass).length,
            1
        );
        insertTemplate = field.insertTemplate();
    ...
    test("switchMode button should consider filtering=false", function () {
        ...
        equal(
            headerTemplate.filter("." + field.insertModeButtonClass).length,
            1,
            "inserting switch button rendered"
        );
        var $modeSwitchButton = headerTemplate.filter(
            "." + field.modeButtonClass
        );
        $modeSwitchButton.trigger("click");
        ok(
            $modeSwitchButton.hasClass(field.modeOnButtonClass),
            "on class is attached"
        );
        equal(
            headerTemplate.filter("." + field.insertModeButtonClass).length,
            1,
            "insert button rendered"
        );
        equal(
            headerTemplate.filter("." + field.searchModeButtonClass).length,
            0,
            "search button not rendered"
        );
        deepEqual(
            optionArgs,
            { name: "inserting", value: true },
            "turn on grid inserting mode"
        );
        $modeSwitchButton.trigger("click");
        ok(
            !$modeSwitchButton.hasClass(field.modeOnButtonClass),
            "on class is detached"
        );
        deepEqual(
            optionArgs,
            { name: "inserting", value: false },
            "turn off grid inserting mode"
        );
    });
    test("switchMode button should consider inserting=false", function () {
        ...
        equal(
            headerTemplate.filter("." + field.searchModeButtonClass).length,
            1,
            "filtering switch button rendered"
        );
        var $modeSwitchButton = headerTemplate.filter(
            "." + field.modeButtonClass
        );
        ...
        equal(
            headerTemplate.filter("." + field.insertModeButtonClass).length,
            0,
            "insert button not rendered"
        );
        deepEqual(
            optionArgs,
            { name: "filtering", value: false },
            "turn off grid filtering mode"
        );
        $modeSwitchButton.trigger("click");
        ok(
            $modeSwitchButton.hasClass(field.modeOnButtonClass),
