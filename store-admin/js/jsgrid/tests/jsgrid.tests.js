$(function () {
    ...
    test("controller methods are $.noop when not specified", function () {
        ...
        $element.jsGrid(gridOptions);
        deepEqual(
            $element.data(JSGRID_DATA_KEY)._controller,
            {
                loadData: $.noop,
                insertItem: $.noop,
                updateItem: $.noop,
    ...
    test("option changing event handlers", function () {
        ...
        equal(
            optionChangingEventArgs.oldValue,
            "testValue",
    ...
    test("common layout rendering", function () {
        ...
        ok(
            $element.children().eq(0).hasClass(grid.gridHeaderClass),
            "grid header"
        );
        ok($element.children().eq(1).hasClass(grid.gridBodyClass), "grid body");
        ok(
            $element.children().eq(2).hasClass(grid.pagerContainerClass),
            "pager container"
        );
        ...
        equal(
            $headerGrid.find("." + grid.headerRowClass).length,
            1,
            "header row"
        );
        equal(
            $headerGrid.find("." + grid.filterRowClass).length,
            1,
            "filter row"
        );
        equal(
            $headerGrid.find("." + grid.insertRowClass).length,
            1,
            "insert row"
        );
        ...
        equal(
            grid._content.parent()[0],
            $bodyGridTable[0],
            "content is tbody in body table"
        );
        equal(
            $bodyGridTable.find("." + grid.noDataRowClass).length,
            1,
            "no data row"
        );
    ...
    test("set default options with setDefaults", function () {
        ...
        equal(
            $element.jsGrid("option", "defaultOption"),
            "test",
            "default option set"
        );
    });
    ...
    asyncTest(
        "loadingIndication=false should not show loading",
        0,
        function () {
            var $element = $("#jsGrid"),
                timeout = 10,
    ...
    test("filter rendering", function () {
        var $element = $("#jsGrid"),
                ...
                fields: [
                    {
                        ...
                        filterTemplate: function () {
                            var result = (this.filterControl = $("<input>")
                                .attr("type", "text")
            ...
            grid = new Grid($element, gridOptions);
        equal(
            grid._filterRow.find(".filter-class").length,
            1,
            "filtercss class is attached"
        );
        equal(
            grid._filterRow.find(".filter-input").length,
            1,
            "filter control rendered"
        );
        equal(
            grid._filterRow.find("." + grid.cellClass).length,
            1,
            "cell class is attached"
        );
        ok(
            grid._filterRow
                .find(".filter-class")
                .hasClass("jsgrid-align-right"),
            "align class is attached"
        );
        ok(
            grid.fields[0].filterControl.is("input[type=text]"),
            "filter control saved in field"
        );
    });
    test("filter get/clear", function () {
                ...
                fields: [
                    {
                        name: "field",
                        filterTemplate: function () {
                            return (this.filterControl = $("<input>").attr(
                                "type",
                                "text"
                            ));
                        },
        ...
        equal(
            grid.fields[0].filterControl.val(),
            "",
            "grid field filterControl cleared"
        );
    });
    test("field without filtering", function () {
        var $element = $("#jsGrid"),
            jsGridFieldConfig = {
                filterTemplate: function () {
                    var result = (this.filterControl = $("<input>").attr(
                        "type",
                        "text"
                    ));
                    return result;
        ...
        deepEqual(
            grid.getFilter(),
            { field2: "test2" },
            "field with filtering=false is not included in filter"
        );
    });
    ...
    test("nodatarow after bind on empty array", function () {
        ...
        grid.option("data", []);
        equal(
            grid._content.find("." + grid.noDataRowClass).length,
            1,
            "no data row rendered"
        );
        equal(
            grid._content.find("." + grid.cellClass).length,
            1,
            "grid cell class attached"
        );
        equal(
            grid._content.text(),
            grid.noDataContent,
            "no data text rendered"
        );
    });
    ...
    test("rows rendered correctly", function () {
        ...
        equal(
            grid._content.find("." + grid.oddRowClass).length,
            2,
            "two odd rows for 3 items"
        );
        equal(
            grid._content.find("." + grid.evenRowClass).length,
            1,
            "one even row for 3 items"
        );
    });
    test("custom rowClass", function () {
        ...
        equal(
            grid._content.find(".custom-row-cls").length,
            3,
            "custom row class"
        );
    });
    ...
    test("rowClick standard handler", function () {
        ...
        equal(
            grid._editingRow.get(0),
            $secondRow.get(0),
            "clicked row is editingRow"
        );
    });
    ...
    test("row selecting with selectedRowClass", function () {
        ...
        ok(
            $secondRow.hasClass(grid.selectedRowClass),
            "mouseenter adds selectedRowClass"
        );
        $secondRow.trigger("mouseleave", $.Event($secondRow));
        ok(
            !$secondRow.hasClass(grid.selectedRowClass),
            "mouseleave removes selectedRowClass"
        );
    });
    test("no row selecting while selection is disabled", function () {
        ...
        ok(
            !$secondRow.hasClass(grid.selectedRowClass),
            "mouseenter doesn't add selectedRowClass"
        );
    });
    test("refreshing and refreshed callbacks", function () {
        ...
        grid.onRefreshed = function (e) {
            ...
            equal(
                grid._content.find("." + grid.oddRowClass).length,
                2,
                "items rendered after refresh"
            );
        };
        grid.option("data", this.testData);
        equal(
            refreshingEventArgs.grid,
            grid,
            "grid provided in args for refreshing event"
        );
        equal(
            refreshedEventArgs.grid,
            grid,
            "grid provided in args for refreshed event"
        );
        equal(
            grid._content.find("." + grid.oddRowClass).length,
            2,
    ...
    test("grid field name used for header if title is not specified", function () {
        var $element = $("#jsGrid"),
            grid = new Grid($element, {
                fields: [new jsGrid.Field({ name: "id" })],
            });
    ...
    test("grid fields header and item rendering", function () {
        ...
        equal(
            grid._headerRow.find("." + grid.headerCellClass).length,
            1,
            "header cell class is attached"
        );
        equal(
            grid._headerRow.find(".header-class").length,
            1,
            "headercss class is attached"
        );
        ok(
            grid._headerRow
                .find(".header-class")
        ...
        equal(
            $secondRow.find(".cell-class").length,
            1,
            "css class added to cell"
        );
        equal(
            $secondRow.find("." + grid.cellClass).length,
            1,
            "cell class is attached"
        );
        ok(
            $secondRow.find(".cell-class").hasClass("jsgrid-align-right"),
            "align class added to cell"
        );
    });
    test("grid field cellRenderer", function () {
        ...
        deepEqual(
            args,
            { value: "test", item: testItem },
            "cellRenderer args provided"
        );
    });
    test("grid field 'visible' option", function () {
        ...
        var gridOptions = {
            editing: true,
            fields: [{ name: "id", visible: false }, { name: "test" }],
        };
        var grid = new Grid($grid, gridOptions);
        equal(
            $grid
                .find("." + grid.noDataRowClass)
                .children()
                .eq(0)
                .prop("colspan"),
            1,
            "no data row colspan only for visible cells"
        );
        grid.option("data", this.testData);
        grid.editItem(this.testData[2]);
        equal(
            $grid.find("." + grid.headerRowClass).children().length,
            1,
            "header single cell"
        );
        equal(
            $grid.find("." + grid.filterRowClass).children().length,
            1,
            "filter single cell"
        );
        equal(
            $grid.find("." + grid.insertRowClass).children().length,
            1,
            "insert single cell"
        );
        equal(
            $grid.find("." + grid.editRowClass).children().length,
            1,
            "edit single cell"
        );
        equal(
            $grid
                .find("." + grid.oddRowClass)
                .eq(0)
                .children().length,
            1,
            "odd data row single cell"
        );
        equal(
            $grid
                .find("." + grid.evenRowClass)
                .eq(0)
                .children().length,
            1,
            "even data row single cell"
        );
    });
    module("inserting");
    test("inserting rendering", function () {
        var $element = $("#jsGrid"),
                ...
                fields: [
                    {
                        ...
                        insertTemplate: function () {
                            var result = (this.insertControl = $("<input>")
                                .attr("type", "text")
            ...
            grid = new Grid($element, gridOptions);
        equal(
            grid._insertRow.find(".insert-class").length,
            1,
            "insertcss class is attached"
        );
        equal(
            grid._insertRow.find(".insert-input").length,
            1,
            "insert control rendered"
        );
        equal(
            grid._insertRow.find("." + grid.cellClass).length,
            1,
            "cell class is attached"
        );
        ok(
        ...
        ok(
            grid.fields[0].insertControl.is("input[type=text]"),
            "insert control saved in field"
        );
    });
    test("field without inserting", function () {
        var $element = $("#jsGrid"),
            jsGridFieldConfig = {
                insertTemplate: function () {
                    var result = (this.insertControl = $("<input>").attr(
                        "type",
                        "text"
                    ));
                    return result;
        ...
        deepEqual(
            grid._getInsertItem(),
            { field2: "test2" },
            "field with inserting=false is not included in inserting item"
        );
    });
    test("insert data", function () {
        ...
        grid.insertItem();
        equal(
            insertingArgs.item.field,
            "test",
            "field is provided in inserting args"
        );
        ...
        equal(
            insertedArgs.item.field,
            "test",
            "field is provided in inserted args"
        );
    });
    test("insertItem accepts item to insert", function () {
        ...
        deepEqual(
            insertedItem,
            itemToInsert,
            "controller insertItem was called with correct item"
        );
    });
    module("editing");
    test("editing rendering", function () {
                ...
                fields: [
                    {
                        ...
                        editTemplate: function (value) {
                            var result = (this.editControl = $("<input>")
                                .attr("type", "text")
                                .val(value)
                                .addClass("edit-input"));
                            return result;
        ...
        equal(
            grid._content.find("." + grid.editRowClass).length,
            0,
            "no edit row after initial rendering"
        );
        grid.editItem(data[0]);
        $editRow = grid._content.find("." + grid.editRowClass);
        equal($editRow.length, 1, "edit row rendered");
        equal(
            $editRow.find(".edit-class").length,
            1,
            "editcss class is attached"
        );
        equal($editRow.find(".edit-input").length, 1, "edit control rendered");
        equal(
            $editRow.find("." + grid.cellClass).length,
            1,
            "cell class is attached"
        );
        ok(
            $editRow.find(".edit-class").hasClass("jsgrid-align-right"),
            "align class is attached"
        );
        ok(
            grid.fields[0].editControl.is("input[type=text]"),
            "edit control saved in field"
        );
    ...
    test("edit item", function () {
        ...
        deepEqual(
            editingArgs.item,
            { field: "value" },
            "item before editing is provided in editing event args"
        );
        equal(
            editingArgs.itemIndex,
            0,
            "itemIndex is provided in editing event args"
        );
        equal(
            editingArgs.row[0],
            editingRow,
            "row element is provided in editing event args"
        );
        grid.fields[0].editControl.val("new value");
        grid.updateItem();
        deepEqual(
            updatingArgs.previousItem,
            { field: "value" },
            "item before editing is provided in updating event args"
        );
        deepEqual(
            updatingArgs.item,
            { field: "new value" },
            "updating item is provided in updating event args"
        );
        equal(
            updatingArgs.itemIndex,
            0,
            "itemIndex is provided in updating event args"
        );
        equal(
            updatingArgs.row[0],
            updatingRow,
            "row element is provided in updating event args"
        );
        ...
        deepEqual(
            updatedArgs.previousItem,
            { field: "value" },
            "item before editing is provided in updated event args"
        );
        deepEqual(
            updatedArgs.item,
            { field: "new value" },
            "updated item is provided in updated event args"
        );
        equal(
            updatedArgs.itemIndex,
            0,
            "itemIndex is provided in updated event args"
        );
        equal(
            updatedArgs.row[0],
            updatedRow,
            "row element is provided in updated event args"
        );
    });
    ...
    test("updateItem accepts item to update and new item", function () {
        ...
        deepEqual(
            updatingItem,
            { field: "new value" },
            "controller updateItem called correctly"
        );
        deepEqual(
            grid.option("data")[0],
            { field: "new value" },
        ...
        var $element = $("#jsGrid"),
            ...
            gridOptions = {
                ...
                editRowRenderer: function (item, itemIndex) {
                    return $("<tr>")
                        .addClass("custom-edit-row")
                        .append($("<td>").text(itemIndex + ":" + item.value));
                },
                fields: [{ name: "value" }],
            },
            grid = new Grid($element, gridOptions);
    ...
    test("delete item", function () {
        ...
        deepEqual(
            deletingArgs.item,
            { field: "value" },
            "field and value is provided in deleting event args"
        );
        equal(
            deletingArgs.itemIndex,
            0,
            "itemIndex is provided in updating event args"
        );
        equal(
            deletingArgs.row.length,
            1,
            "row element is provided in updating event args"
        );
        ...
        deepEqual(
            deletedArgs.item,
            { field: "value" },
            "item is provided in updating event args"
        );
        equal(
            deletedArgs.itemIndex,
            0,
            "itemIndex is provided in updating event args"
        );
        equal(
            deletedArgs.row.length,
            1,
            "row element is provided in updating event args"
        );
    });
    test("deleteItem accepts row", function () {
        ...
        deepEqual(
            deletedItem,
            itemToDelete,
            "controller deleteItem called correctly"
        );
        equal(grid.option("data").length, 0, "data row deleted");
    });
    module("paging");
    test("pager is rendered if necessary", function () {
        ...
        ok(
            grid._pagerContainer.is(":hidden"),
            "pager is hidden when paging=false"
        );
        equal(
            grid._pagerContainer.html(),
            "",
            "pager is not rendered when paging=false"
        );
        grid.option("paging", true);
        ok(
            grid._pagerContainer.is(":visible"),
            "pager is visible when paging=true"
        );
        ok(grid._pagerContainer.html(), "pager is rendered when paging=true");
        grid.option("data", [{}, {}]);
        ok(
            grid._pagerContainer.is(":hidden"),
            "pager is hidden for single page"
        );
        ok(
            grid._pagerContainer.html(),
            "pager is rendered for single page when paging=true"
        );
    });
    ...
    test("pager functionality", function () {
        ...
        equal(
            grid._firstDisplayingPage,
            1,
            "_firstDisplayingPage is initialized"
        );
        pager = grid._pagerContainer;
        equal(
            pager.find("." + grid.currentPageClass).length,
            1,
            "there is one current page"
        );
        ok(
            pager
                .find("." + grid.pageClass)
                .eq(0)
                .hasClass(grid.currentPageClass),
            "first page is current"
        );
        equal(
            pager.find("." + grid.pageClass).length,
            3,
            "three pages displayed"
        );
        equal(
            pager.find("." + grid.pagerNavButtonClass).length,
            5,
            "five nav buttons displayed: Fisrt Prev Next Last ..."
        );
        equal(
            pager.find("." + grid.pagerNavButtonInactiveClass).length,
            2,
            "two nav buttons inactive: Fisrt Prev"
        );
        grid.openPage(2);
        equal(
            pager.find("." + grid.currentPageClass).length,
        ...
        equal(
            pager.find("." + grid.pagerNavButtonClass).length,
            5,
            "five nav buttons displayed: First Prev Next Last and ..."
        );
        equal(
            pageChangedArgs.pageIndex,
            2,
            "onPageChanged callback provides pageIndex in arguments"
        );
        grid.showNextPages();
        ...
        equal(
            grid._firstDisplayingPage,
            3,
            "opening next non-visible page moves first displaying page forward"
        );
        grid.openPage(2);
        equal(
            grid._firstDisplayingPage,
            2,
            "opening prev non-visible page moves first displaying page backward"
        );
    });
    test("pager format", function () {
        var $element = $("#jsGrid"),
            grid = new Grid($element, {
                ...
                pageButtonCount: 1,
                pagerFormat:
                    "a {pageIndex} {first} {prev} {pages} {next} {last} {pageCount} {itemCount} z",
                pagePrevText: "<",
                pageNextText: ">",
                pageFirstText: "<<",
                pageLastText: ">>",
        ...
        grid._refreshPager();
        equal(
            $.trim(grid._pagerContainer.text()),
            "a 2 << < pp2np > >> 3 6 z",
            "pager text follows the format specified"
        );
    });
    ...
    test("loading by page", function () {
        ...
        equal(
            gridData[gridData.length - 1].value,
            7,
            "loaded correct data end value"
        );
        ok(
            pager
                .find("." + grid.pageClass)
    ...
    test("sorting", function () {
        var $element = $("#jsGrid"),
            gridOptions = {
                sorting: true,
                data: [{ value: 3 }, { value: 2 }, { value: 1 }],
                fields: [{ name: "value", sorter: "number" }],
            },
            grid = new Grid($element, gridOptions);
        var gridData = grid.option("data");
    ...
    test("sorting with pageLoading", function () {
        ...
        equal(grid._sortField, grid.fields[0], "sort field is set");
        equal(
            loadFilter.sortOrder,
            "asc",
            "sort direction is provided in loadFilter"
        );
        equal(
            loadFilter.sortField,
            "value",
            "sort field is provided in loadFilter"
        );
        $th.trigger("click");
        equal(grid._sortOrder, "desc", "desc sorting order for second click");
        equal(grid._sortField, grid.fields[0], "sort field is set");
    ...
    test("no sorting for column with sorting = false", function () {
        ...
        var gridOptions = {
            sorting: true,
            data: [{ value: 3 }, { value: 2 }, { value: 1 }],
            fields: [{ name: "value", sorting: false }],
        };
        var grid = new Grid($element, gridOptions);
        var gridData = grid.option("data");
        var $th = grid._headerRow.find("th").eq(0);
        ...
        equal(
            grid._sortField,
            null,
            "sort field is not set for the field with sorting=false"
        );
        equal(gridData[0].value, 3);
        equal(gridData[1].value, 2);
        equal(gridData[2].value, 1);
        equal(
            $th.hasClass(grid.sortableClass),
            false,
            "no sorting css for field with sorting=false"
        );
        equal(
            $th.hasClass(grid.sortAscClass),
            false,
            "no sorting css for field with sorting=false"
        );
    });
    test("sort accepts sorting config", function () {
        ...
        equal(
            grid._sortOrder,
            "desc",
            "desc sorting order for already set asc sorting"
        );
        equal(grid._sortField, grid.fields[0], "sort field is set");
        equal(gridData[0].value, 3);
        equal(gridData[1].value, 2);
    ...
    test("getSorting returns current sorting", function () {
        ...
        deepEqual(
            ...
            "undefined field and order before sorting"
        );
        grid.sort("value");
        deepEqual(
            grid.getSorting(),
            { field: "value", order: "asc" },
            "current sorting returned"
        );
    });
    test("sorting css attached correctly when a field is hidden", function () {
        ...
        var gridOptions = {
            sorting: true,
            data: [],
            fields: [{ name: "field1", visible: false }, { name: "field2" }],
        };
        var grid = new Grid($element, gridOptions);
        var gridData = grid.option("data");
        var $th = grid._headerRow.find("th").eq(0);
        ...
        equal(
            $th.hasClass(grid.sortAscClass),
            true,
            "sorting css is attached to first field"
        );
    });
    ...
    test("rendering", function () {
        ...
        var gridOptions = {
            loadMessage: "",
            data: [{ complexProp: { prop: "test" } }],
            fields: [{ name: "complexProp.prop", title: "" }],
        };
    ...
    test("editing", function () {
        ...
        var gridOptions = {
            editing: true,
            data: [{ complexProp: { prop: "test" } }],
            fields: [{ type: "text", name: "complexProp.prop" }],
        };
        var grid = new Grid($element, gridOptions);
        grid.editItem(gridOptions.data[0]);
        equal(
            grid.fields[0].editControl.val(),
            "test",
            "complex property value set in editor"
        );
    });
    test("should not fail if property is absent", function () {
        ...
        var gridOptions = {
            loadMessage: "",
            data: [{ complexProp: {} }],
            fields: [{ name: "complexProp.subprop.prop", title: "" }],
        };
        new Grid($element, gridOptions);
    ...
    test("inserting", function () {
        ...
        deepEqual(
            ...
            "inserting item has complex properties"
        );
    });
    ...
    test("updating", function () {
        ...
        deepEqual(
            updatingItem,
            { complexProp: { prop: "test" } },
            "updating item has complex properties"
        );
    });
    test("update nested prop", function () {
        ...
        var gridOptions = {
            editing: true,
            data: [{ prop: { subprop1: "test1", subprop2: "test2" } }],
            fields: [
        ...
        deepEqual(
            updatingItem,
            expectedUpdatingItem,
            "updating item has nested properties"
        );
    });
    test("updating deeply nested prop", function () {
        ...
        var gridOptions = {
            editing: true,
            data: [{ complexProp: { subprop1: { another_prop: "test" } } }],
            fields: [
        ...
        deepEqual(
            updatingItem,
            expectedUpdatingItem,
            "updating item has deeply nested properties"
        );
        deepEqual(
            previousItem,
            expectedPreviousItem,
            "previous item preserved correctly"
        );
    });
    module("validation");
    ...
    test("insertItem rejected when data is not valid", function () {
        ...
        var gridOptions = {
            ...
            fields: [{ type: "text", name: "Name", validate: true }],
        };
        var grid = new Grid($element, gridOptions);
        grid.fields[0].insertControl.val("test");
        grid.insertItem()
    ...
    test("invalidClass is attached on invalid cell on inserting", function () {
        ...
        grid.insertItem();
        ok(
            $insertCell.hasClass(grid.invalidClass),
            "invalid class is attached"
        );
        equal(
            $insertCell.attr("title"),
            "Error",
            "cell tooltip contains error message"
        );
    });
    test("onItemInvalid callback", function () {
        ...
        equal(
            onItemInvalidCalled,
            1,
            "onItemInvalid is called, when item data is invalid"
        );
        deepEqual(
            onItemInvalidArgs,
            {
                grid: grid,
                errors: [{ field: grid.fields[0], message: "Error" }],
                item: { Name: "" },
                itemIndex: -1,
                row: grid._insertRow,
            },
        ...
        equal(
            onItemInvalidCalled,
            1,
            "onItemInvalid was not called, when data is valid"
        );
    });
    test("invalidNotify", function () {
        ...
        equal(
            invalidNotifyCalled,
            1,
            "invalidNotify is called, when item data is invalid"
        );
        deepEqual(
            invalidNotifyArgs,
            {
        ...
        equal(
            invalidNotifyCalled,
            1,
            "invalidNotify was not called, when data is valid"
        );
    });
    test("invalidMessage", function () {
        ...
        try {
            ...
            equal(
                ...
                "message contains invalidMessage and field error messages"
            );
        } finally {
    ...
    test("updateItem should call validation.validate", function () {
        ...
        var gridOptions = {
            ...
            fields: [{ type: "text", name: "Name", validate: "required" }],
        };
        var grid = new Grid($element, gridOptions);
        grid.editItem(gridOptions.data[0]);
        grid.fields[0].editControl.val("test");
    ...
    test("should pass undefined and null arguments to the renderer", function () {
        ...
        Grid.prototype.renderTemplate(renderer, context, {
            arg1: undefined,
            arg2: null,
            arg3: "test",
        });
