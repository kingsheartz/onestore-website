...
<i class="fas fa-poll" style="font-size: 24px;padding-right: 12px" aria-hidden="true"></i>New Orders </h4>
...
<button class="prbt"onclick="$('#order<?=$row['new_ordered_products_id']?>').print();">Print to Pdf</button>
 ...
 <button  type="button"  data-toggle="collapse" href="#myNavbarc<?=$row['new_ordered_products_id']?>" >
    ...
    <span style="padding: 5px;font-size:16px">   Customer details</span>   <span class="amount">  Total Amount <i class="fas fa-rupee" style="padding-left: 12px;"></i><?=$row['sub_total']?>    </span>           
...
<td>   <?=$row['new_ordered_products_id']?></td>
 ...
 <td align="center">  
  ...
  src="../images/<?=$row['category_id']?>/<?=$row['sub_category_id']?>/<?=$row['item_description_id']?>.jpg"> </div>
...
<tr><th>  Item Quantity</th>

<td><?=$row['item_quantity']?></td></tr>

<tr><th>Total</th>

<td><?=$row['total_amt']?></td></tr>
...
<button name="deliver" class="orderbutton"

 onclick="ordchn()">Mark As Shipped</button>
