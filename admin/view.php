...
.left-arrow{
  position: absolute;
    ...
    z-index: 1;
    height: 100px;
    font-size: 24px;    border-top-left-radius: 5px;
...
.right-arrow{
  position: absolute;
    ...
    z-index: 1;
    height: 100px;
    font-size: 24px;    border-top-right-radius: 5px;
...
.difhed button{
 ...
 <body>
            ...
            <div class="table1">
 <h4 style="margin-top: 30px;margin-bottom:50px;border-bottom:  1px solid#E3E3E3;padding:10px;"><i class="fa fa-pencil-square-o" style="font-size: 24px;padding-right: 12px"aria-hidden="true"></i>Edit Products</h4>
...
<?php
require "../pdo.php";
$query="SELECT * FROM item JOIN item_description ON item.item_id=item_description.item_id  GROUP BY item_description.item_id ORDER BY item_description.item_description_id DESC LIMIT 8";
$st=$pdo->query($query);
$ct='ab';
?>
<div class="difcat ">
...
<button class="right-arrow" onclick="moveright('difrow<?=$ct?>')" style="display: none;"><i class="fas fa-chevron-left"></i></button>
...
justify-content: center;height: 200px;width:100%;background: white;text-align: center;"><img class="image" align="middle" src="../images/<?=$row['category_id']?>/<?=$row['sub_category_id']?>/<?=$row['item_description_id']?>.jpg"></div>
  <div class="middle">
     <form id="C<?=$row['item_description_id']?>" method="post" action="change.php" name="<?=$row['item_description_id']?>">
  ...
  <button onclick="showupda(<?=$row['item_description_id']?>)"class="updation"><i class="fa fa-pencil-square-o" style="font-size: 24px;padding-right: 12px"aria-hidden="true"></i>Change</button>
   </form>

     <form id="D<?=$row['item_description_id']?>" method="post" action="productuplo.php" name="1<?=$row['item_description_id']?>">
  ...
  <button onclick="showupda('1<?=$row['item_description_id']?>')"class="updation"><i class="fas fa-upload" style="font-size: 24px;padding-right: 12px"aria-hidden="true"></i>Upload</button>
   </form>
</div>
...
$query="SELECT * FROM item JOIN item_description ON item.item_id=item_description.item_id where item.category_id=$ct GROUP BY item_description.item_id";
$st=$pdo->query($query);
$product=$st->rowCount();
if ($product==0) {
continue;}
...
while ($row=$st->fetch(PDO::FETCH_ASSOC)) {     
  ...
  justify-content: center;height: 200px;width:100%;background: white;text-align: center;"><img class="image" align="middle" src="../images/<?=$row['category_id']?>/<?=$row['sub_category_id']?>/<?=$row['item_description_id']?>.jpg"></div>
    ...
    <button onclick="showupda(<?=$row['item_description_id']?>)"class="updation"><i class="fa fa-pencil-square-o" style="font-size: 24px;padding-right: 12px"aria-hidden="true"></i>Change</button>
       ...
       <form id="a<?=$row['item_description_id']?>" method="post" action="productuplo.php" name="1<?=$row['item_description_id']?>">
      <input type="hidden" name="uppr_id" value="<?=$row['item_description_id']?>">
            <input type="hidden" name="upitem_id" value="<?=$row['item_id']?>">
